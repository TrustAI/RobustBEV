...
CONFIG=/home/fu/workspace/RobustBEV/zoo/PETR/projects/configs/petr/petr_vovnet_gridmask_p4_800x320.py
CHECKPOINT=/datasets/bev_models/petr_vov_800.pth
GPU_ID="0"
...
CUDA_VISIBLE_DEVICES=$GPU_ID \
    ...
    --save-result --save-name petr_vov_hsb

CUDA_VISIBLE_DEVICES=$GPU_ID \
    python3 ./tools/eval.py $CONFIG $CHECKPOINT --eval bbox\
    --kernal-size $motion --perturb-type motion \
    --max-evaluation $nquery --max-iteration $iter --max-deep $deep  \
    --save-result --save-name petr_vov_mb

CUDA_VISIBLE_DEVICES=$GPU_ID \
    python3 ./tools/eval.py $CONFIG $CHECKPOINT --eval bbox\
    --perturb-type geometry --shift $shift --scale $scale\
    --max-evaluation $nquery --max-iteration $iter --max-deep $deep  \
    --save-result --save-name petr_vov_scsft