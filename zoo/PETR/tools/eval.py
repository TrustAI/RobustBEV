...
sys.path.append("./")
sys.path.append("../../")
from verify_utils import (LowBoundedDIRECT_potential, CentreDistance,
                        get_task_bound)
...
def get_args():
    ...
    # motion blur
    parser.add_argument('--kernal-size', default=0.0, type=int,
...
###################################################
    ...
    for i, data in enumerate(data_loader):
        if i+1 < start_frame: continue
        elif i+1 > end_frame: break
        # Before verification, check and record the model's prediction
        with torch.no_grad():
        ...
        # Re-implement the found worst case perturbation
        pertubation.append(direct_solver.optimal_result())

        if args.perturb_type == 'motion':
            optimal = np.reshape(
                            direct_solver.optimal_result(), 
                            (task.nb_camera, -1))
            tmp_img = task.functional_perturb(
                                task.rgb_images, optimal)
            # for kk in range(task.nb_camera):
            #     save_image(task.rgb_images[kk], f'./ori_img_{kk}.png')
            #     save_image(task._normlise_img(
            #         tmp_img[kk] * task.img_std[:,None,None]
            #         +task.img_mean[:,None,None])[0][[2,1,0],...], 
            #         f'./tmp_img_{kk}.png')
        else:
            optimal = np.reshape(direct_solver.optimal_result(), 
                        (-1,len(bound)//task.nb_camera))
            tmp_img = torch.zeros_like(task.rgb_images)          
            for kk in range(task.nb_camera):
                min_max_v = task.min_max_vaules[kk]
                # save_image(task.rgb_images[kk], f'./ori_img_{kk}.png')
                tmp_img[kk] = task.functional_perturb(task.rgb_images[kk],
                                                    optimal[kk],min_max_v)
                # save_image(task._normlise_img(
                #     tmp_img[kk]+IMG_MEAN[:,None,None])[0][[2,1,0],...], 
                #     f'./tmp_img_{kk}.png')
